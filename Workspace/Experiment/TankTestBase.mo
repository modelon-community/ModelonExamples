...
model TankTestBase
    .Modelon.ThermoFluid.Volumes.TwoPhaseTankSimpleWall storageTank(enable_externalHeatPort = true,N_vaporPort = 1,N_liquidPort = 1,redeclare replaceable package Medium = .VaporCycle.Media.Naturals.HydrogenMbwr,redeclare replaceable .Modelon.ThermoFluid.Volumes.SubComponents.TankShapes.HorizontalCylinderEllipsoid geo(r_internal = r_inner,s_end = r_inner,L = L_cylinder,t_wall = t_insulation),redeclare replaceable .Modelon.ThermoFluid.Volumes.HeatTransfer.ConstantCoefficient internalConvection(alpha_dry_prescribed = fill(alpha_wall,2),alpha_wet_prescribed = fill(alpha_wall,2)),K_cond_surface = 1e-3,g_surface = 4,g_external = k_insulation / t_insulation,k_wall = k_insulation,p_start = p_start,initFromLevel = false,V_liq_start = V_liq_start) annotation(Placement(transformation(extent = {{-30.0,6.0},{-10.0,26.0}},origin = {0.0,0.0},rotation = 0.0)));
